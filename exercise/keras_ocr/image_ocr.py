...
# TODO test_func是 K.function 的中间层定义，通过传入输入数据，获得输出结果
def decode_batch(test_func, word_batch):
    ...
    for j in range(out.shape[0]):
        ...
        show = False
        #TODO  测试结果
        if np.sum(np.where(np.argmax(out[j, 2:], 1) == 27, True, False)) != out[j].shape[0] -2:
            show = True
...
class VizCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        #保存模型,可以用此方法在模型损失率或准确率不再降低或提升时，中断运行
...
def train(run_name, start_epoch, stop_epoch, img_w):
    ...
    train_data = img_gen.next_train()
    # for i in train_data:
    #     print(i)
    print("absolute_max_string_len",img_gen.absolute_max_string_len)

    print("get_output_size", img_gen.get_output_size())
    # print("max_string_len", img_gen.max_string_len)
    print("blank_label", img_gen.blank_label)
    print("minibatch_size", img_gen.minibatch_size)
    ...
    #TODO 自定义一层输出，在ctc_lambda_func中自己处理损失函数，返回的是一个损失函数
    loss_out = Lambda(
    ...
    #TODO K.function用于获得中间某一层的结果,此例子中是为了获得y_pred的结果
    test_func = K.function([input_data], [y_pred])

    viz_cb = VizCallback(run_name, test_func, img_gen.next_val())

    """
    val_acc = validation_accuracy :验证的准确率
    val_loss = validation_loss: 验证的损失率
    """
    model.fit_generator(
...
if __name__ == '__main__':
    ...
    fdir = "/Users/konglinghong/Downloads/wordlists/"
    img_h = 60
    img_w = 160
    words_per_epoch = 16000
    val_split = 0.2
    ...
    print(128/ (pool_size ** 2) - 2)
    print("datas",data)
    cnt = 0;
    for i in data:
        d = i[0]
        # print(d["the_input"].shape)
        # print(d["the_labels"].shape)
        # print(d["input_length"])
        # print(d["label_length"])
        print(d)
        break
