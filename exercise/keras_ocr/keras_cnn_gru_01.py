    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if max(acc) > 0.8 or epoch >= SAVE_THRESHOLD:
            self.model.save_weights(
                os.path.join(self.output_dir, 'epoch_%02d.h5' % (epoch)))
...
def train(epoch_num=None, name=MODEL_NAME):
    ...
    for i in range(2):
        ...
        x = Conv2D(filters=2 ** (4+i), kernel_size=(3, 3), padding="same", activation='relu', kernel_initializer='he_normal')(x)
        x = Conv2D(filters=2 ** (4+i), kernel_size=(3, 3), padding="same", activation='relu', kernel_initializer='he_normal')(x)
    ...
    # gru_2 = TimeDistributed(BatchNormalization(axis=1))(gru_2)
    # gru_2b = TimeDistributed(BatchNormalization(axis=1))(gru_2b)
    ...
    # evaluator_func.
    # base_model.summary()
    evaluator = Evaluate(validation_func=evaluator_func,val_seq=val_obj,name="keras_cnn_gru_01")
